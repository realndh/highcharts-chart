...
import Highcharts from './es-modules/masters/highcharts.src.js'
...
export const HighchartsPolymer = {
    ...
    return class extends superClass {
      ...
      __createChart(namespace) {
        const getFirstOrAxisConfig = axis => axis instanceof Array
          ...
          : axis;
        let tempX_Axis = _extends(this.vsTime ? {
          type: 'datetime',
          tickPixelInterval: 150
        } : {}, getFirstOrAxisConfig(this.xAxis));
        ...
        (titleConfig => {
          titleConfig.text = titleConfig.text || this._getAxisLabel('X')
        })(getFirstOrAxisConfig(this.xAxis).title || {});
        (titleConfig => {
          titleConfig.text = titleConfig.text || this._getAxisLabel('Y')
        })(getFirstOrAxisConfig(this.yAxis).title || {});
        let Series = this.data.length && this.data[0].data instanceof Array ? this.data : [{
          name: (this.label || this.yLabel || this.xLabel),
        ...
        this._set_chart(new Highcharts[namespace || "Chart"](_extends(true, {}, {
          chart: _extends({
            ...
            events: {
              click: function (e) {
                __app._fire("chart-click", {e: e, chart: this, component: __app})
              },
              load: function (e) {
                __app._fire("chart-load", {e: e, chart: this, component: __app})
              },
              beforePrint: function (e) {
                __app._fire("before-print", {e: e, chart: this, component: __app})
              },
              afterPrint: function (e) {
                __app._fire("after-print", {e: e, chart: this, component: __app})
              },
              addSeries: function (e) {
                __app._fire("series-added", {e: e, chart: this, component: __app})
              },
              // drilldown: function (e) {
              //   __app._fire("drill-down", {e: e, chart: this, component: __app})
              // },
              // drillup: function (e) {
              //   __app._fire("drill-up", {e: e, chart: this, component: __app})
              // },
              // drillupall: function (e) {
              //   __app._fire("drill-up-all", {e: e, chart: this, component: __app})
              // },
              selection: function (e) {
                __app._fire("selection", {e: e, chart: this, component: __app})
              },
              redraw: function (e) {
                __app._fire("redraw", {e: e, chart: this, component: __app})
              },
              render: function (e) {
                __app._fire("render", {e: e, chart: this, component: __app})
              },
            }
      ...
      setData(x, z) {
        ...
        if (!isArr) {
          x = _extends({name: (this.label || this.yLabel || this.xLabel), colorByPoint: this.colorByPoint}, x)
        }
      ...
      addData(x, y, z, drillable) {
        this.pushData(x, y, z, true, drillable)
      }
      ...
      addSeries(name, data, colorByPoint, otherOptions) {
        this._chart.addSeries(_extends({
          name: name,
          data: (data || []),
          colorByPoint: typeof colorByPoint == "undefined" ? this.colorByPoint : colorByPoint
        }, typeof otherOptions == "object" ? otherOptions : {}), true)
      }

      addDrillSeries(point, data, name) {
        this._chart.addSeriesAsDrilldown(point, {name: name, data: data})
      }

      removeSeries(z, redraw) {
        const s = this._getSeries(z);
        s.remove(redraw);
        return s
      }
      ...
      reRender() {
        if (!this._chart) return
        this._set_chart(new Highcharts.Chart(this._chart.options));
        this.setData(this.data);
        this.__lateBinders()
      }

      downloadAs(name, otherOptions) {
        if (!this._chart) return
        this._chart.exportChart(_extends({filename: name}, otherOptions))
      }
      ...
      _dataUpdate(d) {
        this.setData(d.slice ? d.slice(0) : d)
      }

      _warn(err, c) {
        console.warn("%c[highcharts-chart" + (c ? "::" + c : '') + "]", "font-weight:bold;background-color:yellow", err)
      }
      ...
      _legendOptionsUpdate(o) {
        if (!this._chart) return
        this._chart.legend.update(o)
      }

      _tooltipOptionsUpdate(o) {
        if (!this._chart) return
        this._chart.tooltip.update(o)
      }

      _annotationsUpdate(o) {
        if (!this._chart) return
        this._chart.annotations.update(o)
      }
      ...
      _chartUpdate(o) {
        if (!this._chart) return
        Object.keys(o || {}).length && this._warn("Not doing what you wanted, maybe you meant plotOptions?", "chartOptions");
        this._chart.update({chart: o})
      }
      ...
      _cbpUpdate() {
        this.updateSeries("colorByPoint", this.colorByPoint)
      }
      ...
      _labelUpdate() {
        if (!this._chart) return
        this._xAxisTitleUpdate();
        this._yAxisTitleUpdate()
      }
      ...
      _getRenderer() {
        ...
        return this._chart.renderer
      }

      //Other Bindings
      _getAxisLabel(dim) {
        let l = dim.toLowerCase(), u = dim.toUpperCase(), lab = this[l + "Label"] || this.label;
        if (lab == u + "-Axis") {
          lab = this.label || lab
        }
      ...
      _returnDummySeries() {
        ...
        return {isDummy: true, setData: f, addPoint: f, update: f, remove: f}
      }

      __lateBinders() {
        const me = this;
        Object.keys(this.__lateBindCallbacks || {}).forEach(function (cb) {
          me[cb]()
        })
      }
  ...
  ChartBehavior: superClass => {
    return class extends superClass {
      constructor() {
        super();
        this.__addLateBinder("_vsTimeUpdate")
      }
      ...
      _vsTimeUpdate(a) {
        if (!this._chart || typeof a == "undefined") return
        this.destroy();
        this.ready()
      }
      ...
      _checkSelected() {
        if (!this._chart) return
        let points = this._chart.getSelectedPoints();
        this._setSelected(!!points.length);
        this._setSelectedPoints(points)
      }
    }
  }
}