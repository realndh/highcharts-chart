...
PTCS.List = class extends PTCS.BehaviorValidate(PTCS.BehaviorFocus(
    ...
    ready() {
        ...
        ['mousedown', 'keydown', 'touchstart'].forEach(evName => this.addEventListener(evName, interacted, {capture: true}));
    ...
    static get $parts() {
        if (!this._$parts) {
            this._$parts = [
                ...
                'item-checkbox', 'item-value', /*'state-value',*/ 'item-meta',
    ...
    _validateSelectedValue(extraValidation) {
        if (this.multiSelect) { // in this case selected value is not relevant
            return true;
        }
