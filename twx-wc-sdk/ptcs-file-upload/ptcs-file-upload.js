...
PTCS.FileUpload = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
    static get template() {
        return html`
            ...
            <div part=repo>
                <ptcs-dropdown part="repos-list" label="File Repository" items="[[repositoryList]]" disabled="[[disabled]]"
                    selected-value="{{repo}}" tabindex\$="[[_delegatedFocus]]">
                </ptcs-dropdown>
                <ptcs-textfield part="path" label="Path" text="{{path}}" disabled="[[disabled]]" tabindex\$="[[_delegatedFocus]]"></ptcs-textfield>
            </div>
            ...
            <ptcs-file-upload-list part="file-upload-list" id="file-upload-list" upload-manager="[[uploadManager]]"
                ...
                extra-validation="[[extraValidation]]" external-validity="[[externalValidity]]" validation-output="{{validationOutput}}">
