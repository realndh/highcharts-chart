...
const navKeys = new Set(['Home', 'End', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Enter', ' ']);
...
const DataFilter = class extends PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)) {
    static get is() {
    ...
    __updateTranslations() {
        ...
        if (filterOperatorEl) {
            filterOperatorEl.items = filterOperators;
        }
    }
    ...
    _createOnDemandFilterOperator() {
        ...
        filterOperator.items = filterOperators;
    ...
    _focusEv() {
        ...
        if (!this._focusEl || focusIndex(this._focusEl, focusable) === -1) {
            this._focusEl = focusable[0];
        }
        if (this._focusEl) {
            this._focusEl.focus();
        }
    ...
    _mouseDown() {
        ...
        requestAnimationFrame(() => {
            this._focusEl = this.focusElement;
        });
    }
    ...
    _keyDown(ev) {
        ...
        if (key === 'Tab') {
            if (ev.shiftKey) {
                // Prevent backwards Tab navigation from stopping on this (ptcs-pagination) element
                if (!delegateToPrev(this)) {
                    (this.focusElement || this).blur();
                }
                ev.preventDefault();
            } else if (this._focusableSlotted.length) {
        ...
        const focusEl = this.focusElement;
        if (!focusEl) {
            return;
        }
        ...
        const focusable = this.focusableElements;
        let index = focusIndex(focusEl, focusable);
        if (index === -1) {
            // Out of sync. Reset
            index = 0;
        }
        ...
        switch (ev.key) {
            case 'Home':
                newFocusEl = focusable[0];
                break;
            case 'End':
                newFocusEl = focusable[focusable.length - 1];
                break;
            case 'ArrowLeft':
            case 'ArrowUp':
                newFocusEl = focusable[index - 1];
                break;
            case 'ArrowRight':
            case 'ArrowDown':
                newFocusEl = focusable[index + 1];
