...
|--------- |------|-------------|---------------------------|
|menuType|String|Defines the _type_ of the Menu bar, how subitems are displayed. Should be either `'flyout'` or `'nested'`.
...
|moreItemsLabel|String|The string to use for the text of the virtual "More..." entry (default: `'More...'`)||
|moreItemsIcon|String|The icon to use for the virtual _More items..._ entry (default: `'cds:icon_more_horizontal'`)||
|brandingItem|String|The data to use for the branding/footer section. Uses the same format as the "main" data, there should be a property called `label` for the text, `icon` for the (optional) icon and additional `logo` that overrides both `label` and `icon` on expanded mode. Any other data can also be added, this items is returned to the callback when the user clicks the footer.||
