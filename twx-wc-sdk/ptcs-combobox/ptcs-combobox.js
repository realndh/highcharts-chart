...
const _startsWith = (item, selectorF, text) => {
    if (item) {
        return selectorF(item).toLowerCase().startsWith(text.toLowerCase());
    }
    return false;
};
...
PTCS.ComboBox = class extends PTCS.BehaviorValidate(PTCS.BehaviorTooltip(PTCS.BehaviorFocus(
    ...
    <div id="select" part="select-box">
      ...
      <ptcs-dropdown id="dropdown" part="drop-down" selector="[[selector]]" items="[[items]]" combobox-mode disabled="[[disabled]]"
        ...
        multi-line="[[multiLine]]" row-height="[[rowHeight]]"
    ...
    ready() {
        ...
        this.$.dropdown.addEventListener('dropdown-closed', ev => {
            if (ev.detail !== this._textInternal) {
                let match;
                this.__textSetFromList = true;

                if (this._textInternal) {
                    match = this._getItemMatchedText(this._textInternal);
                }

                // If any match corresponding, overwrite "match" variable with the current input value
                if (!match) {
                    match = this._textInternal;
                }
                // If we found match in the items list or the current input are not empty text
                if (match) {
                    this._updateSelected(match);
                } else if (ev.detail) {
                    this._textInternal = ev.detail;
                }
            }
    ...
    _getSuggestion(text) {
        const match = this._checkMatch(text);
        if (match) {
            return _getItemSuggestion(match, this._selectorF, text.length);
        }
    ...
    _openPopup(filterString) {
        ...
        this._popup.filter = _createFilterFunction(this._selectorF);
        this._popup.hideFilter = true;

        this._popup.tabindex = '0';
