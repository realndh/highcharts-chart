...
function createSingleSelectHeader() {
    return (dataMgr) => {
        ...
        const selected = dm => typeof dm.selected === 'number' || (Array.isArray(dm.selected) && dm.selected.length);

        // Something has changed in the data manager, so the radio needs to update
        el.selectionChanged = function() {
            el.checked = !selected(dataMgr);
        };

        el.addEventListener('checked-changed', function(ev) {
            // If user selects this button, the current selection should be cleared
            if (ev.detail.value && selected(dataMgr)) {
                dataMgr.selected = null;
            }
        });

        el.selectionChanged(); // Initialize state
        cntr.appendChild(el);
        return cntr;
    };
}
