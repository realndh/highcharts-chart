...
PTCS.Grid = class extends PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement))) {
    static get template() {
        return html`
            <style>
           :host {
            ...
            <ptcs-core-grid id="grid" tabindex\$="[[_delegatedFocus]]" part="core-grid"
                ...
                disable-row="[[disableRow]]" disable-child-rows="[[disableChildRows]]"
                ...
                select-label="[[selectLabel]]" cancel-label="[[cancelLabel]]"
    ...
    static get properties() {
        return {
            ...
            _hideToolbar: {
                ...
                computed: '_computeHideToolbar(showFilter, showResetButton, showEditButton, showDeleteRowButton, showAddRowButton, showExpandAll, asynchronousNodeLoading, alwaysExpanded, columnsMenuOptions)'
            },
    ...
    static get observers() {
        return [
            ...
            // eslint-disable-next-line max-len
            '_initToolbar(resetButtonText, editButtonText, saveEditButtonText, cancelEditButtonText, addRowButtonText, deleteRowButtonText, displayButtonText, resetButtonType)',
        ...
        this.__toolbarValue = ev => {
            switch (ev.detail.action.id) {
                case 'toggle-edit':
                    ...
                    break;
                default:
                    console.log('toolbar value: ' + ev.detail.action.id + ' = ' + JSON.stringify(ev.detail.value));
            }
        };
    }
    ...
    ready() {
        ...
        // When was the keyboard last pressed?
        this.addEventListener('keydown', () => {
            this._lastKeyEventTime = Date.now();
    ...
    _columnsDisplayModeChanged(mode) {
        ...
        if (mode === 'open') {
            ...
            // Enable keyboard focus if column reorder form was opened using the keyboard
            if ((Date.now() - this._lastKeyEventTime) <= 25) {
                this.$['columns-display'].focusEnabled = true;
            }
    ...
    _computeHideToolbar(showFilter, showResetButton, showEditButton, showDeleteRowButton, showAddRowButton, showExpandAll,
        asynchronousNodeLoading, alwaysExpanded, columnsMenuOptions) {
    ...
    _hasEditableRowsChanged(_hasEditableRows) {
        if (this.$.toolbar) {
            this.$.toolbar.setDisabled('add-row', !_hasEditableRows);
        }
    }
