...
PTCS.CoreGrid = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
    static get template() {
        ...
        [part=hit-area]:focus, ptcs-icon[part=edit-control]:focus-within, [part=invalid-icon]:focus, ptcs-icon[part=tree-toggle-icon]:focus-within {
            outline: var(--ptcs-focus-overlay--border-style, solid) var(--ptcs-focus-overlay--border-width, 2px) var(--ptcs-focus-overlay--border-color, #0094c8);
        ...
        <div id="header" class="row" part="header" ondragstart="return false" hidden\$="[[hideHeader]]"></div>
                    ...
                    rows.forEach(def => {
                        ...
                        if (def.editable && this._editCells && this.editControlVisibility !== 'never') {
                            ...
                            if (editControl !== null) {
                                ...
                                const toggleIcon = div.querySelector('[part~=tree-toggle-icon]');
                                if (toggleIcon) {
                                    toggleIcon.setAttribute('grid-action', '');
                                }
    ...
    _disableCell(node, isDisabled) {
        node.childNodes.forEach(childNode => {
            if (childNode.tagName !== 'PTCS-ICON') {
                if (childNode.classList.contains('tree-toggle')) {
                    childNode.disabled = false;
                } else {
                    childNode.disabled = isDisabled;
                }
            } else {
                childNode.ariaDisabled = isDisabled;
            }
        });
    }
    ...
    __setCellWidthWithConstraints(resizedCell, newWidth) {
        ...
        let cellIndex = PTCS.getChildIndex(resizedCell);
        ...
        columns.forEach((col) => {
            ...
            if (this._resizeColumns) {
                if (col.nonresizable) {
                    ...
                    let minW = PTCS.cssDecodeSize(col.minWidth, this);
                    let maxW = PTCS.cssDecodeSize(col.maxWidth, this);

                    const requiredMin = col.sortable ? 34 : this.__resizerHitArea;
                    minW = (isNaN(minW) || minW < requiredMin) ? requiredMin : minW;
    ...
    **********************************************************************/
    ...
    __changeColumnWidth(dir) {
        const newWidth = this._resizedFocusedCell.getBoundingClientRect().width + (dir === 'right' ? 2 : -2);
    ...
    __moveFocusToNextCell(isGridEmpty) {
        const focusedCell = this._focusedCell.nextSibling;
    ...
    __processKeyArrowRight({focusEl, isInHeader, isGridEmpty, cellActions, navModes, lockedAction}) {
        if (focusEl === this._resizerFocusHandle && this.$['resizer-focus'].hasAttribute('selected')) {
            this.__changeColumnWidth('right');
            return true;
        }
    ...
    __processKeyArrowLeft({focusEl, isInHeader, isGridEmpty, cellActions, navModes, lockedAction}) {
        if (focusEl === this._resizerFocusHandle && this.$['resizer-focus'].hasAttribute('selected')) {
            this.__changeColumnWidth('left');
            return true;
        }
    ...
    __processKeyEnter({ev, focusEl, cellActions}) {
        ...
        if (this.data.selectMethod === 'single' && this._focusedRow && this._focusedRow.hasOwnProperty('index') && !this._focusedAction) {
            const value = this._focusedRow.index;
        ...
        // Toggle state?
        const toggleIcon = this._focusedCell.querySelector('[part~=tree-toggle-icon]');
    ...
    _launchEditor(baseIndex, item, field, colNo, cellEl, title, updateButtonText) {
        ...
        if (this.cancelButtonText) {
            this._gridEditor.cancelButtonText = this.cancelButtonText;
        }

        // Calendar labels
        if (this.dateLabel) {
            this._gridEditor.dateLabel = this.dateLabel;
        }

        if (this.monthLabel) {
            this._gridEditor.monthLabel = this.monthLabel;
        }

        if (this.yearLabel) {
            this._gridEditor.yearLabel = this.yearLabel;
        }

        if (this.hoursLabel) {
            this._gridEditor.hoursLabel = this.hoursLabel;
        }

        if (this.minutesLabel) {
            this._gridEditor.minutesLabel = this.minutesLabel;
        }

        if (this.secondsLabel) {
            this._gridEditor.secondsLabel = this.secondsLabel;
        }

        if (this.meridiemLabel) {
            this._gridEditor.meridiemLabel = this.meridiemLabel;
        }

        if (this.selectLabel) {
            this._gridEditor.selectLabel = this.selectLabel;
        }

        if (this.cancelLabel) {
            this._gridEditor.cancelLabel = this.cancelLabel;
        }
    ...
    _editDone(ev, focus) {
        ...
        // Inform data manager about changes
        let baseIndex = this._gridEditor.baseIndex;
        const update = ev.detail && ev.detail.values;
        if (!update) {
            if (!this._isItemUpdated(baseIndex, this._gridEditor.field)) {
                ...
                this.dispatchEvent(new CustomEvent('edit-item-cancelled', {
                    ...
                    detail:   {
                        baseIndex,
                        field: this._gridEditor.field,
                        item:  this._gridEditor.item}}));
            }
            return;
        }
        ...
        if (baseIndex === newRowBaseIndex) {
            ...
            const index = this.data.insertItem(Object.assign({}, this._gridEditor.item, update), undefined, ev.detail.validation);
            baseIndex = this.data.baseIndex(index);

            requestAnimationFrame(() => {
                this.$.chunker.setFocusRowIndex(index);
            });
        } else {
            const index = this.data.translateBaseIndexToIndex(baseIndex);
            if (this._gridEditor.item !== this.data.item(index)) {
                console.error('data item has changed during editing');
                return;
            }

            this.data.updateItem(index, update, ev.detail.validation);
        }
        ...
        this.dispatchEvent(new CustomEvent('edit-item', {
            ...
            detail:   {
                baseIndex,
                update,
                validation: ev.detail.validation}}));
        ...
        switch (this.view.editLevel) {
            case 'cell':
            case 'row':
                this.data.submitIfValid(baseIndex, this._gridEditor.field);
        }
        ...
        if (!this._isItemUpdated(baseIndex, this._gridEditor.field) && itemUpdated) {
    ...
    addRow(item) {
        ...
        this._launchEditor(newRowBaseIndex, _item, undefined, -1, null, this.rowEditFormAddTitle, this.addButtonText);
    }
};
