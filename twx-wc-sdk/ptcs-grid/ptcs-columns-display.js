...
PTCS.ColumnsDisplay = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
        ...
        return html`
            ...
            <div class="column-defs" id="columns" tabindex\$="[[_delegatedFocus]]">
                    ...
                    <template>
                        <div class="column-def" part="column-def">
                            <ptcs-icon part="col-icon" hidden$="[[_isIconHidden(options)]]" icon="cds:icon_drag_handle_mini"></ptcs-icon>
                                <ptcs-checkbox part="col-checkbox" hidden$="[[_isCheckboxHidden(options)]]"
                ...
                <div id="draggable" class="column-def draggable" part="column-def">
                    <ptcs-icon part="col-icon" hidden$="[[_isIconHidden(options)]]" icon="cds:icon_drag_handle_mini"></ptcs-icon>
                    <ptcs-checkbox id="draggable-checkbox" part="col-checkbox" hidden$="[[_isCheckboxHidden(options)]]"
    ...
    static get properties() {
        return {
            ...
            // Has the keyboard been pressed? Otherwise, hide focus
            focusEnabled: {
                type: Boolean
            },
    ...
    _focusedElement() {
        if (!this.focusEnabled || this._focusedColumnIx < 0 || !Array.isArray(this._columns) || this._focusedColumnIx > this._columns.length - 1) {
    ...
    _modeChanged(mode) {
        this.focusEnabled = false; // And a keyboard event is needed for the focus to activate
    ...
    _keyDown(ev) {
        ...
        this.focusEnabled = true; // Keyboard has been pressed
