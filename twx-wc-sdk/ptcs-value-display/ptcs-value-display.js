...
PTCS.ValueDisplay = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
    static get is() {
        ...
        if (this.$.valueroot.scrollHeight > 0) { // Ready?
            ...
            if (this._valueType !== 'image') {
                ...
                if (valuecontainerHeight > 0 && !this._showAll) {
                    this._overflow = (valuecontainerHeight + this.labelHeight) > this.$.valuedisplayarea.clientHeight;
                }
            }
        }
    }
    ...
    close() {
        if (this._showpopup) {
            ...
            document.body.removeChild(this._dialog); // Remove the popup from DOM
            this._dialog = undefined;

            // Remove the modal background overlay from DOM
            const mdl = document.body.querySelector('ptcs-modal-overlay');
            mdl.parentNode.removeChild(mdl); // Remove the modal overlay from DOM
