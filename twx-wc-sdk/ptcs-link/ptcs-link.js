...
PTCS.Link = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
    static get template() {
        return html`
    <style>
      :host {
    ...
    <a part="link" href\$="[[_compute_href(disabled, href)]]" target\$="[[_compute_target(target)]]"
      ...
      variant="[[labelVariant]]" exportparts\$="[[_exportparts]]"
...
`;
    ...
    static get properties() {
        return {
            ...
            // If this property is assigned, then the ptcs-label within this ptcs-link will get this variant
            labelVariant: {
                type: String
            },
    ...
    ready() {
        ...
        // Make sure the labelVariant has a value
        if (this.labelVariant === undefined) {
    ...
    _onClick(ev) {
        if (!this.href || this.disabled || !PTCS.validateURL(this.href) || this.isIDE) {
            ev.preventDefault();
            return;
        }
        ...
        this.dispatchEvent(new CustomEvent('a-click', {bubbles: true, composed: true, detail: {a: this.shadowRoot.querySelector('a'), originalEvent: ev}}));
