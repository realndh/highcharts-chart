...
PTCS.Pagination = class extends PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)) {
    ...
    static get template() {
        return html`
            ...
            <ptcs-hbar id="page-break-control-and-total-results-container" part="page-break-and-total-results-container"
                       start center>
                <ptcs-label  part="total" hidden\$="[[!showTotalResults]]"
                    id="total" disable-tooltip
                    label="[[totalNumberOfElements]]"></ptcs-label>
    ...
    stringPerPageChanged(stringPerPage) {
        const str = (stringPerPage + '').trim();
        const placeholder = '__ResultsDropdown__';
        // Arrange the flexbox children of ptcs-hbar in the order the tokens appear in the localized string
        if (str.startsWith(placeholder)) {
            this.$['main-drop-down'].style.order = '3';
            this.$['string-per-page'].style.order = '4';
            this._stringPerPage = str.slice(placeholder.length).trim();
        } else if (str.endsWith(placeholder)) {
            this.$['string-per-page'].style.order = '3';
            this.$['main-drop-down'].style.order = '4';
            this._stringPerPage = str.slice(0, str.length - placeholder.length).trim();
        } else if (!str.includes(placeholder)) {
            console.warn('required placeholder ' + placeholder + ' missing in property stringPerPage');
        }
    }

    stringResultsChanged(stringResults) {
        const str = (stringResults + '').trim();
        const placeholder = '__TotalResults__';
        // Arrange the flexbox children of ptcs-hbar in the order the tokens appear in the localized string
        if (str.startsWith(placeholder)) {
            this.$.total.style.order = '1';
            this.$['total-results-label'].style.order = '2';
            this._stringResults = str.slice(placeholder.length).trim();
        } else if (str.endsWith(placeholder)) {
            this.$['total-results-label'].style.order = '1';
            this.$.total.style.order = '2';
            this._stringResults = str.slice(0, str.length - placeholder.length).trim();
        } else if (!str.includes(placeholder)) {
            console.warn('required placeholder ' + placeholder + ' missing in property stringResults');
        }
    }
    ...
    get focusableElements() {
        ...
        if (this.showPageBreak) {
            result.push(this.$['main-drop-down']);
        }

        // Collect all buttons in the carousel
        result.push(...[...this.$.carousel.shadowRoot.querySelectorAll('ptcs-button:not([hidden])')]);

        if (this.showDirectLink) {
            // Get the direct link textfield
            result.push(this.$['input-number'].shadowRoot.querySelector('ptcs-textfield'));
        }
        return result;
    }
    ...
    _keyDown(ev) {
        ...
        if (this._focusEl.tagName === 'PTCS-TEXTFIELD') {
            switch (ev.key) {
                case 'ArrowLeft':
                case 'Home':
                    if (this._focusEl.selectionEnd > 0) {
                        return; // Ignore unless cursor is at start of text
                    }
                    break;
                case 'End':
                case 'ArrowRight':
                    if (this._focusEl.selectionStart < this._focusEl.text.length) {
