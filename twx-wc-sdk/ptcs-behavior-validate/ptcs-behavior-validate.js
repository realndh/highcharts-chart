...
const VALIDATED_PENDING = 5; // Validating, and last result is still not retrieved
...
PTCS.BehaviorValidate = superClass => {
    return class extends superClass {
        ...
        static get observers() {
            return [
                '_messageFormatChanged(_externalMsgs.*)'
            ];
        }
        ...
        _clearPropMsg(propName) {
            if (this._externalMsgs[propName]) {
                const clone = Object.assign({}, this._externalMsgs);
                delete clone[propName];
                this.set('_externalMsgs', clone);
            }
        }
        ...
        _updateValidity(propName) {
            ...
            if (typeof this[validateMethod] === 'function') {
                ...
                this.__validationPromiseController[validateMethod].validationTimeoutId = setTimeout(() => {
                    ...
                    this._validatedProps[propName] = VALIDATED_PENDING;
                    this._markValidity();
                            ...
                            switch (val) {
                                ...
                                default:
                                    if ((Array.isArray(val))) {
                                        this._validatedProps[propName] = VALIDATED_FALSE;
                                        this.set(`_externalMsgs.${propName}`, val.filter(msg => msg));
                                    } else {
                                        this._validatedProps[propName] = VALIDATED_IGNORE;
        ...
        _doMarkValidity() {
            ...
            if (validity) {
                this.validity = (this._stayUnvalidated || validity === Validity.PENDING) ? Validity.UNVALIDATED : validity;
            } else {
        ...
        __validityChanged(validity, oldValue) {
            if (this.validity === null) {
                this.validity = oldValue; // null was set by this.removeAttribute('validity'). So restore prev value.
                return; // rest of the changes will be done by observer call triggered by previous line
            }
            ...
            // we have manually set/remove validity attribute
            let vAttr = this.getAttribute('validity') || undefined;
            if (vAttr !== validity) {
                if (!validity) {
                    this.removeAttribute('validity');
                    // This is PROBABLY not needed---but just in case there is some CSS that hides something so
                    // that an existing validation message goes away, we'll allow any observer to trigger...
                    this._validationChangeNo++;
                    return; // this.validity was change to null now. No need to continue.
                }
