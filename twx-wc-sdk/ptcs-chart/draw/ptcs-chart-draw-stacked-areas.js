...
export class DrawStackedAreas extends DrawStack {
    constructor(seriesIxs, data, method, order, curve, markers, showValues, formatValues) {
    ...
    drawMarkers({el, xScale, yScale, flipAxes, selectionMgr}) {
        ...
        const yPos = d => (_yScale(d[1]) + _yScale(d[0])) / 2;
    ...
    drawValues({el, xScale, yScale, flipAxes}) {
        ...
        function drawSeriesValues([index, seriesIx, showValues, formatValue, marker, markerSize]) {
            ...
            drawValues({el: this, data: data[index], label, legend, xPos, yPos, showValues, marker, markerSize, flipAxes});
        }
