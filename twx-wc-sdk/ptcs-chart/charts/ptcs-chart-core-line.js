...
function computeMaxPoints(xRange) {
    const num = xRange / 3;

    // Never use more than 512 sample points (which is already a bit slow)
...
PTCS.ChartCoreLine = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement)))) {
    ...
    __filterStackedData(_stackedData) {
        ...
        const maxPoints = computeMaxPoints(maxX);
        ...
        if (x1 > 0 || x2 < xOrg.length - 1) {
            ...
            if (sampledIx.length > maxPoints) {
                // Sample down data set
                sampledIx = sampleArray(sampledIx, maxPoints, sampleOutlier);
            }
        } else if (x.length > maxPoints) {
            // Sample down data set
            sampledIx = sampleArray(x.map((v, i) => i), maxPoints, sampleOutlier);
        }
        ...
        const process = (_data, yScale, deltaY, mapSerie) => {
            ...
            _data.forEach((serie, six) => {
                ...
                if (maxPoints < _serie.length) {
                    _serie = sampleArray(_serie, maxPoints, compareSamples);
                }
