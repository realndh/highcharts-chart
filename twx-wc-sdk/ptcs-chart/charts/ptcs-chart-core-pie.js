...
PTCS.ChartCorePie = class extends PTCS.BehaviorTooltip(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement))) {
    static get template() {
        return html`
        <style>
        :host {
        ...
        <div id="chart" part="chart" ondragstart="return false"
            on-mouseout="_closeTooltip" on-click="_clickOnChart">
            <ptcs-part-observer>
            <div id="invalid">
                <ptcs-icon part="invalid-icon" icon="cds:icon_error"></ptcs-icon>
                <ptcs-label label="[[invalidLabel]]" horizontal-alignment="center"></ptcs-label>
            </div>
            <svg>
                <defs id="defs"></defs>
                <g id="pie" part="pie"></g>
    ...
    ready() {
        ...
        this.addEventListener('blur', ev => this._blurEv(ev));
    ...
    _rebuildModel() {
        ...
        this.invalidData = data.length > 0 && !dataValidity;
    ...
    _mapModelToDOM() {
        ...
        if ((this.showValues && (this.valuePos === 'out' || this.valuePos === 'out with line'))) {
        ...
        const _mousedown = (d, i) => {
            const elValue = this._findValueEl(this._series[d.index] + 1);
            if (!elValue) {
                return;
            }
            elValue.setAttribute('slice-pressed', '');
            this._mouseupFunc = () => {
                document.removeEventListener('mouseup', this._mouseupFunc);
                this._mouseupFunc = undefined;
                elValue.removeAttribute('slice-pressed');
            };
    ...
    _updateInsideLabel(index) {
        ...
        if (index !== undefined) {
            ...
            index = this._focus.__data__.index;
            _value = this._fetchValueByIndex(index);
            _label = this.data[index][0];
        }
    ...
    selectedChanged(index, selected) {
        ...
        if (elSlice) {
            if (selected) {
                elSlice.setAttribute('selected', '');
            } else {
                elSlice.removeAttribute('selected');
            }
            elSlice.style.transform = highlightSelected ? elSlice.__highlightTranslate : '';
        }
        if (elValue) {
            if (selected) {
                elValue.setAttribute('selected', '');
            } else {
                elValue.removeAttribute('selected');
            }
