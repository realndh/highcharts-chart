...
PTCS.Datepicker = class extends PTCS.BehaviorValidate(PTCS.BehaviorTooltip(PTCS.BehaviorFocus(
    ...
    static get properties() {
        return {
            ...
            __validateNo: {
                ...
                value:    0,
                // eslint-disable-next-line max-len
                validate: '_validateDatepicker(dateTime, fromDate, toDate, dateRangeSelection, _showTime, _displaySeconds, _twelveHourClock, required, min, max,' +
                    'minStartDate, maxStartDate, minEndDate, maxEndDate, extraValidation)'
            }
        };
    }
    ...
    _dfTabindex(_delegatedFocus, _noSpaceForMessage, forceHide = false) {
        return _noSpaceForMessage || forceHide ? '-1' : _delegatedFocus;
    }
    ...
    _validateDatepicker(dateTime, fromDate, toDate, dateRangeSelection, _showTime, _displaySeconds, _twelveHourClock,
        required, min, max, minStartDate, maxStartDate, minEndDate, maxEndDate, extraValidation) {
        let messages = [];

        // Start with true if validation is needed, otherwise undefiend
        let result = (required || (!dateRangeSelection && (min || max)) ||
