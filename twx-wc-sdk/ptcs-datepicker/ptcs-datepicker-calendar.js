...
PTCS.DatepickerCalendar = class extends PTCS.BehaviorFocus(PTCS.BehaviorStyleable(PTCS.ThemableMixin(PolymerElement))) {
    ...
    static get observers() {
        return [
            ...
            '_daysChanged(_selectedYear, _selectedMonth, datePresentedByDots, dateRangeSelectionCalendar, min, max, minStartDate, maxStartDate, minEndDate, maxEndDate)',
    ...
    _outOfRangeFunc() {
        if (this.dateRangeSelectionCalendar) {
            ...
            if ((min1 || max1 || min2 || max2) && (!min1 || !max2 || min1.isBefore(max2)) && (!max1 || !min2 || max1.isBefore(min2))) {
                return day => {
